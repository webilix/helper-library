...
exports.BANK = {
    findCard: (card) => {
        if (!validator_library_1.Validator.VALUE.isString(card) ||
            card.length < 6 ||
            card.length > 16 ||
            (card.length === 16 && !validator_library_1.Validator.STRING.isBankCard(card)))
            return null;
    ...
    getList: () => list.map((l) => ({ id: l.id, title: l.title })),
};
